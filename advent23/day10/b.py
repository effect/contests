...
def get_connected_exclude_loop(row, col):
	result = []
	dir = ((-1, 0), (1, 0), (0, -1), (0, 1))
	for (dr, dc) in dir:
		r = row + dr
		c = col + dc
		if in_bounds(r, c) and (r, c) not in loop:
			result.append((r, c))
	return result
...
q = Queue()
q.put((srow, scol))
loop = set()  # the main loop by extended coordinates
while not q.empty():
	i, j = q.get()
	adj = get_connected(i, j)
	for (ni, nj) in adj:
		...
		if d[ni][nj] == d[i][j] + 1:
			q.put((ni, nj))
		loop.add((ext_i, ext_j))
		loop.add((ext_ni, ext_nj))
		loop.add((mid_i, mid_j))
...
de = [[-1] * ncols for _ in range(nrows)]
q = Queue()
...
while not q.empty():
	i, j = q.get()
	adj = get_connected_exclude_loop(i, j)
	for (ni, nj) in adj:
		if de[ni][nj] == -1:
			de[ni][nj] = de[i][j] + 1
			q.put((ni, nj))
...
for i in range(nrows):
	for j in range(ncols):
		if lines[i][j] != ' ':
			if de[i][j] == -1 and (i, j) not in loop:
				result += 1
print(result)
